...
public class PostController {
    ...
    public ResponseEntity<SuccessResponseDto<List<Post>>> getPosts(
        ...
        return ResponseEntity.ok().body(new SuccessResponseDto<>(postService.getAllByKeywordContaining(page, size, keyword)));
    }
    ...
    public ResponseEntity<SuccessResponseDto<Boolean>> likePost(@PathVariable int postId) throws Exception {
        int userId = 2;
        return ResponseEntity.ok().body(new SuccessResponseDto<>(postService.likePost(userId, postId)));
    }
//    @PostMapping("/api/post/{postId}/like")
