...
public class PostService {
    ...
    public Post getPostById(int postId) throws Exception {
        ...
        Post post = postRepository.findById(postId).orElseThrow(() -> new NotFoundException("존재하지 않는 게시글 입니다."));
    ...
    public List<Post> getAllByKeywordContaining(int page, int size,String keyword) throws Exception {
        ...
        List<Post> posts = postRepository.findAllByKeyword(startIndex, size, keyword).orElseThrow(() -> new NotFoundException("검색됨 정보가 없습니다."));
        return posts;
    }
    ...
    public Boolean likePost(int postId, int userId) throws Exception {
        PostLike postLike = PostLike.builder()
                ...
                .userId(userId)
                .build();
        return postLikeRepository.save(postLike).orElseThrow(() -> new CustomDuplicateKeyException("",Map.of("message","해당 게시글을 이미 like 처리하였습니다.")));
    }
}
